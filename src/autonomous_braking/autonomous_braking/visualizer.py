...
class AEBVisualizer(Node):
    ...
    def _state_cb(self, msg):
        try:
            ...
            self.aeb           = s.get('aeb', {})
            self.pid           = s.get('pid', {})
            self.brake_pid     = s.get('brake_pid', {})
...
# ═══════════════════════════════════════════════════════════════
#  LONGITUDINAL PID PANEL  (top-right: speed + braking controllers)
# ═══════════════════════════════════════════════════════════════

def _draw_pid_terms(screen, x, y, w, pid_dict, font_xs):
    """Render the P / I / D contributions of one controller on a row."""
    terms = [("P", pid_dict.get('p', 0.0)),
             ("I", pid_dict.get('i', 0.0)),
             ("D", pid_dict.get('d', 0.0))]
    seg_w = w // 3
    for idx, (name, val) in enumerate(terms):
        screen.blit(font_xs.render(f"{name} {val:+.2f}", True, C['text_dim']),
                    (x + idx * seg_w, y))


def draw_pid_panel(screen, ego, pid, brake_pid, font_md, font_sm, font_xs):
    """
    Top-right panel showing the two longitudinal PID controllers:
      • SPEED PID — tracks target speed, output = throttle (gas) / ease-off
      • BRAKE PID — tracks a safe gap, output = brake force (0..100%)
    """
    panel_w = 244
    panel_h = 250
    px = WIDTH - panel_w - 12
    ...
    # Title
    title = font_sm.render("◈  LONGITUDINAL  PID", True, C['text_bright'])
    screen.blit(title, (px + panel_w // 2 - title.get_width() // 2, py + 5))

    # ── SECTION A: SPEED PID (throttle) ──────────────────────────
    screen.blit(font_xs.render("SPEED PID  →  THROTTLE", True, C['text_label']),
                (bar_x, py + 24))

    target = pid.get('target_speed_kmh', 0.0)
    current = ego.get('speed_kmh', 0.0)
    s_err = pid.get('error_kmh', 0.0)
    s_out = pid.get('output', 0.0)

    screen.blit(font_xs.render("TARGET", True, C['text_dim']), (bar_x, py + 38))
    screen.blit(font_md.render(f"{target:.0f}", True, C['safe']), (bar_x, py + 49))
    screen.blit(font_xs.render("CURRENT", True, C['text_dim']), (bar_x + 120, py + 38))
    screen.blit(font_md.render(f"{current:.0f}", True, C['text_bright']), (bar_x + 120, py + 49))
    screen.blit(font_xs.render("km/h", True, C['text_dim']), (bar_x + 188, py + 56))

    # Throttle/brake bar: centre = 0, right = gas, left = ease-off
    bar_y = py + 78
    bar_h = 11
    mid_x = bar_x + bar_w // 2
    pygame.draw.rect(screen, C['dash_border'], (bar_x, bar_y, bar_w, bar_h), border_radius=3)
    out_norm = max(-1.0, min(1.0, s_out / PHYSICS.max_acceleration))
    fill_px = int(abs(out_norm) * (bar_w // 2))
    if s_out >= 0:
        pygame.draw.rect(screen, C['safe'], (mid_x, bar_y, fill_px, bar_h), border_radius=3)
    else:
        pygame.draw.rect(screen, C['warning'], (mid_x - fill_px, bar_y, fill_px, bar_h), border_radius=3)
    pygame.draw.line(screen, C['text_mid'], (mid_x, bar_y - 2), (mid_x, bar_y + bar_h + 2), 1)
    sign = "+" if s_err >= 0 else ""
    screen.blit(font_xs.render(f"err {sign}{s_err:.1f}", True, C['text_mid']),
                (bar_x, bar_y + bar_h + 3))
    so_txt = font_xs.render(f"{s_out:+.2f} m/s²", True, C['text_mid'])
    screen.blit(so_txt, (bar_x + bar_w - so_txt.get_width(), bar_y + bar_h + 3))
    _draw_pid_terms(screen, bar_x, bar_y + bar_h + 16, bar_w, pid, font_xs)

    # Divider
    div_y = py + 138
    pygame.draw.line(screen, C['dash_divider'], (px + 8, div_y), (px + panel_w - 8, div_y), 1)

    # ── SECTION B: BRAKE PID (gap) ───────────────────────────────
    screen.blit(font_xs.render("BRAKE PID  →  STOP GAP", True, C['text_label']),
                (bar_x, div_y + 6))

    want = brake_pid.get('desired_gap', 0.0)
    actual = brake_pid.get('actual_gap', -1)
    g_err = brake_pid.get('gap_error', 0.0)
    b_out = brake_pid.get('output', 0.0)
    actual_str = f"{actual:.0f}" if actual >= 0 else "—"

    screen.blit(font_xs.render("WANT", True, C['text_dim']), (bar_x, div_y + 20))
    screen.blit(font_md.render(f"{want:.0f}", True, C['caution']), (bar_x, div_y + 31))
    screen.blit(font_xs.render("ACTUAL", True, C['text_dim']), (bar_x + 120, div_y + 20))
    actual_col = C['danger'] if (actual >= 0 and actual < want) else C['text_bright']
    screen.blit(font_md.render(actual_str, True, actual_col), (bar_x + 120, div_y + 31))
    screen.blit(font_xs.render("m", True, C['text_dim']), (bar_x + 168, div_y + 38))

    # Brake force bar (0..100%), left→right
    bb_y = div_y + 60
    bb_h = 11
    pygame.draw.rect(screen, C['dash_border'], (bar_x, bb_y, bar_w, bb_h), border_radius=3)
    bfill = int(max(0.0, min(1.0, b_out)) * bar_w)
    bcol = (C['emergency'] if b_out > 0.85
            else C['braking'] if b_out > 0.15 else C['safe'])
    if bfill > 0:
        pygame.draw.rect(screen, bcol, (bar_x, bb_y, bfill, bb_h), border_radius=3)
    sign = "+" if g_err >= 0 else ""
    screen.blit(font_xs.render(f"gap err {sign}{g_err:.1f}m", True, C['text_mid']),
                (bar_x, bb_y + bb_h + 3))
    bo_txt = font_xs.render(f"BRAKE {int(b_out*100)}%", True, bcol)
    screen.blit(bo_txt, (bar_x + bar_w - bo_txt.get_width(), bb_y + bb_h + 3))
    _draw_pid_terms(screen, bar_x, bb_y + bb_h + 16, bar_w, brake_pid, font_xs)
...
def main(args=None):
    ...
    while running:
        ...
        draw_pid_panel(screen, ego, pid, brake_pid, font_md, font_sm, font_xs)
