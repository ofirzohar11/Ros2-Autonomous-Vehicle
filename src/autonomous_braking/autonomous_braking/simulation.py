...
from autonomous_braking.config import (
    PHYSICS, SAFETY, LIDAR_CFG, ROAD, TRAFFIC,
...
class WorldSimulation(Node):
    ...
    def _aeb_decision(self):
        ...
        if nearest_dist > SAFETY.safe_distance:
            ...
            if do_lc:
                ...
                self.brake_intensity = 0.1
                self.target_speed = PHYSICS.cruise_speed_kmh / 3.6 * 0.85
                if not self.is_changing_lane:
                    self._try_lane_change(all_objects)
            else:
                # Moving vehicle ahead → ACC following: brake proportionally
                speed_diff = max(0.0, self.ego.vy - nearest_obj.vy)
                self.brake_intensity = min(0.55, speed_diff / PHYSICS.max_deceleration)
                self.target_speed = nearest_obj.vy * 0.97  # stay slightly behind

        elif nearest_dist > SAFETY.warning_distance:
            # ── WARNING (20–35 m) ─────────────────────────────────────────
            self.status = VehicleStatus.WARNING
            if do_lc:
                self.brake_intensity = 0.35
                self.target_speed = PHYSICS.cruise_speed_kmh / 3.6 * 0.55
                if not self.is_changing_lane:
                    self._try_lane_change(all_objects)
            else:
                # Moving vehicle → stronger following brake
                speed_diff = max(0.0, self.ego.vy - nearest_obj.vy)
                self.brake_intensity = min(0.7, max(0.25,
                    speed_diff / PHYSICS.max_deceleration))
                self.target_speed = nearest_obj.vy * 0.93

        elif nearest_dist > SAFETY.danger_distance:
            # ── DANGER (12–20 m) ──────────────────────────────────────────
            self.status = VehicleStatus.DANGER
            self.brake_intensity = 0.65
        ...
        if self.ttc < SAFETY.ttc_emergency and nearest_dist < SAFETY.warning_distance:
            ...
            self.status = VehicleStatus.DANGER
            self.brake_intensity = max(self.brake_intensity, 0.6)
    ...
    def _update_ego_physics(self):
        speed_diff = self.target_speed - self.ego.vy

        if self.brake_intensity > 0:
            decel = PHYSICS.comfort_deceleration + (PHYSICS.max_deceleration - PHYSICS.comfort_deceleration) * self.brake_intensity
            self.ego.vy = max(0, self.ego.vy - decel * self.dt)
        elif speed_diff > 0.5:
            accel = min(PHYSICS.max_acceleration, speed_diff * 2.0)
            self.ego.vy = min(PHYSICS.max_speed_kmh / 3.6, self.ego.vy + accel * self.dt)
        elif speed_diff < -0.5:
            self.ego.vy = max(0, self.ego.vy - PHYSICS.comfort_deceleration * 0.3 * self.dt)
